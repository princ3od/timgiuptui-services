...
class Provider:
    ...
    def get_similar_articles_of(self, id: str) -> Optional[list[SimilarArticle]]:
        ...
        for similar_article_id, similar_article in article.similar_articles.items():
            similar_articles.append(similar_article.copy(update={"id": similar_article_id}))
        sorted_similar_articles = sorted(similar_articles, key=lambda x: x.similarity, reverse=True)
